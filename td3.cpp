...
double* remove_from_array(double* array,
                          int &current_size,
                          int &max_size) {
  // Remove last elm without changing current and max size
  ...
  for (int i = 0; i < global_telemetry_current_size; i += 3) {
    for (int j = i + 3; j < global_telemetry_current_size; j += 3) {
      if (global_telemetry[i] > global_telemetry[j]) {
        double tmp_t = global_telemetry[i];
        double tmp_x = global_telemetry[i+1];
        double tmp_y = global_telemetry[i+2];
        global_telemetry[i] = global_telemetry[j];
        global_telemetry[i+1] = global_telemetry[j+1];
        global_telemetry[i+2] = global_telemetry[j+2];
        global_telemetry[j] = tmp_t;
        global_telemetry[j+1] = tmp_x;
        global_telemetry[j+2] = tmp_y;
      }
    }
  }
}
